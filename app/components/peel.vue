...
export function createPeel(
   ...
   *
   * `uniforms` is an index signature and this project compiles with
   * `noUncheckedIndexedAccess`, so a direct read is typed
   * `WebGLUniformLocation | undefined` while every `gl.uniform*` overload takes
   * `WebGLUniformLocation | null`. Normalising here keeps that conversion in
   * one place, and returning null for an unknown name is also the correct
   * runtime behaviour: WebGL treats a null location as a no-op, which is what
   * should happen if a uniform was optimised out of the shader.
   */
  const u = (name: string): WebGLUniformLocation | null => sheet.uniforms[name] ?? null;
  ...
  capture = () => {
    ...
    try {
      ...
      // A capture can fail while the subtree is mid-layout or the context is
      // lost. The previous texture stays on screen and the next paint retries,
    ...
    if (probeCtx) {
      ...
      while (el) {
        ...
        if (bg && bg !== "transparent") {
          ...
          // Destructuring a Uint8ClampedArray yields `number | undefined`
          // under noUncheckedIndexedAccess. A one-pixel RGBA read always has
          // four entries, so the fallbacks are unreachable — they exist to
          // satisfy the compiler rather than to handle a real case.
  ...
  function render() {
    ...
    gl!.uniform1f(u("uPeel"), peel.a);
    gl!.uniform1f(u("uReveal"), Math.max(config.reveal, 0));
    gl!.uniform1f(u("uCurl"), Math.max(config.curl, 1));
    gl!.uniform1f(u("uBow"), config.bow);
    gl!.uniform1f(u("uFocal"), Math.max(config.perspective, 200));
    gl!.uniform1f(u("uShade"), config.shade);
    gl!.uniform1f(u("uZone"), Math.max(config.zone, 1));
    gl!.uniform1f(u("uBulge"), Math.max(config.bulge, 0));
    ...
    gl!.uniform1f(u("uCross"), side < 1.5 ? h : w);
    gl!.uniform1f(
      u("uSpan"),
      config.shineDistance > 0 ? config.shineDistance : side < 1.5 ? w : h,
    );
    gl!.uniform2f(u("uPointer"), pointer.su, pointer.sv);
...
/* eslint-disable import/first -- ESLint reads both <script> blocks as a single
   module, so these imports appear to follow the top block's code. They cannot
   be hoisted any further: they belong to <script setup>. */
