...
const icons: Record<string, string[]> = {
	...
	//
	// Literally the same, now — the first two entries are `eye` character for
	// character, with the slash added. They used to be redrawn as arcs broken
	// around the slash, which was prettier but put the eye's top at y=5 where
	// `eye` has it at 6: pressing the reveal button moved the eye up a pixel
	// and back down again, on an icon whose whole job is to sit still while its
	// state changes. Sharing the path data makes that impossible by
	// construction rather than by careful redrawing.
	eyeOff: [
		"M2.5 12s3.6-6 9.5-6 9.5 6 9.5 6-3.6 6-9.5 6-9.5-6-9.5-6Z",
		"M12 14.6a2.6 2.6 0 1 0 0-5.2 2.6 2.6 0 0 0 0 5.2Z",
		"M3.5 3.5l17 17",
