<template>
	<!-- Two columns on Settings, one centred column on Account, and in source
	     order either way. Nothing is reordered to make the heights line up:
	     these panels are read top to bottom the first time somebody opens the
	     page, and pairing them by height would shuffle that for a tidier bottom
	     edge, which is the wrong trade. -->
	<div v-if="data" class="dashSettings" :class="{ 'dashSettings--stack': isAccount }">
