...
- **Supabase** (`@supabase/supabase-js`) — auth and database, accessed via `$supabase` plugin
...
app/
  ...
  plugins/             # $gsap, $lenis, $supabase — client-side only
...
| `SUPABASE_ANON_KEY` | Supabase anon key (client-safe) |
